<?php
    ...
    class App{
        function __construct(){
            $url = isset($_GET["url"]) ? $_GET["url"] : null;//obtenemos todo lo que venga despues de localhost/elegant.sv y validamos que si el campo viene vacio este sera null

            $url = rtrim($url ?? '', '/');//limpiamos la url para evitar parametros como localhost/elegant.sv//////Controller
            $url = explode('/',$url);//convertimos a un arreglo todo lo que venga despues de la raiz del proyecto

            if(empty($url[0])){
                $archivoController = 'controllers/Tienda_OnlineController.php';
                require_once($archivoController);
                $controller = new Tienda_OnlineController();
                $controller->loadModel('Produc');
